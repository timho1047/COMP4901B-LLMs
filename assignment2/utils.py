...
def is_single_turn_conversation(messages: List[Dict[str, str]]) -> bool:
    roles = [msg["role"] for msg in messages]
    assistant_turns = roles.count("assistant")
    user_turns = roles.count("user")
    system_turns = roles.count("system")
    total_turns = len(roles)
    return (
        assistant_turns == 1
        and user_turns == 1
        and system_turns <= 1
        and total_turns == (assistant_turns + user_turns + system_turns)
    )

def _normalize_turn(turn) -> Optional[Dict[str, str]]:
    if "role" in turn:
        raw_role = str(turn["role"]).lower()
        content = turn.get("content", turn.get("value", ""))
    else:
        raw_role = str(turn.get("from", "")).lower()
...
def normalize_conversation(
    ...
    normalized = system_prefix[:]
    for rnd in rounds:
        normalized.extend(rnd)
